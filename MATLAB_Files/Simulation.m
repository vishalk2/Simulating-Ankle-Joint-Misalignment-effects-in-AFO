...
% Parameters
theta_range = -20:2:20; % Ankle Dorsiflexion angle range
...
for MCase=1:4
    switch MCase
        case 1 % Case-1 : Anterior-Posterior Misalignments
            ...
            SP_res_AP = zeros(9,21);
            SP_res_PD = zeros(9,21);
            TP_res_AP = zeros(9,21);
            TP_res_PD = zeros(9,21);
            ...
            for d=1:length(Disp) % Iterating for every Misalignment case
                ...
                AP_SP = zeros(1,21); % Anterior-Posterior Relative Motion for Strap Points for each angle
                PD_SP = zeros(1,21); % Proximal-Distal Relative Motion for Strap Points for each angle
                AP_TP = zeros(1,21); % Anterior-Posterior Relative Motion for Touch Points for each angle
                PD_TP = zeros(1,21); % Proximal-Distal Relative Motion for Touch Points for each angle
...
function relativeMotionPlot(P_res,theta_range,MCase,Mis_Type,YLim,YLabel,Title)
    ...
    figure;
    plot(theta_range,P_res(1,:),'k-o','MarkerSize',4)
    hold on;
    plot(theta_range,P_res(2,:),'k-+','MarkerSize',4)
    plot(theta_range,P_res(3,:),'k-*','MarkerSize',4)
    plot(theta_range,P_res(4,:),'k-x','MarkerSize',4)
    plot(theta_range,P_res(5,:),'k-','MarkerSize',4)
    plot(theta_range,P_res(6,:),'k->','MarkerSize',4)
    plot(theta_range,P_res(7,:),'k-<','MarkerSize',4)
    plot(theta_range,P_res(8,:),'k-^','MarkerSize',4)
    plot(theta_range,P_res(9,:),'k-v','MarkerSize',4)
