...
for MCase=1:4
    switch MCase
        case 1 % Case-1 : Anterior-Posterior Misalignments
            ...
            FP_res_AP = zeros(9,5);
            FP_res_PD = zeros(9,5);
            FSP_res_AP = zeros(9,5);
            FSP_res_PD = zeros(9,5);
            ...
            for d=1:length(Disp) % Iterating for every Misalignment case
                ...
                AP_FP = zeros(1,5); % Anterior-Posterior Relative Motion for Foot Points for each angle
                PD_FP = zeros(1,5); % Proximal-Distal Relative Motion for Foot Points for each angle
                AP_FSP = zeros(1,5);% Anterior-Posterior Relative Motion for Foot Strap Points for each angle
                PD_FSP = zeros(1,5);% Proximal-Distal Relative Motion for Foot Strap Points for each angle
                ...
                for th=1:length(theta_range) % Iterating for every Ankle Dorsiflexion Angle
                    ...
                    [TP1_ACS,TP2_ACS,TP1_OCS,TP2_OCS] = parallelPoints(-theta,deltaX,deltaY,TP1,TP2); % Touch Points
                    [FP1_ACS,FP2_ACS,FP1_OCS,FP2_OCS] = parallelPoints(theta,deltaX,deltaY,FP1,FP2); % Foot Points
                    [FSP0_ACS,FSP1_ACS,FSP0_OCS,FSP1_OCS] = strapPoints(theta,deltaX,deltaY,FSP0,FSP1); % Foot Strap Points
                    ...
                    % Illustration Plots                  
                    % Model-1
                    illustrationPlot1(ACS,FE,FT,Knee,SP0_OCS,SP1_ACS,SP1_OCS,Knee_ACS,TP1_ACS,TP2_ACS,TP1_OCS,TP2_OCS,deltaX,deltaY,OCS)
                    % Model-2
                    illustrationPlot2(ACS,FE,FT,Knee,FSP0_OCS,FSP1_ACS,FSP1_OCS,FE_ACS,FT_ACS,FP1_ACS,FP2_ACS,FP1_OCS,FP2_OCS,deltaX,deltaY,OCS)
            ...
            relativeMotionPlot(FSP_res_AP,theta_range,MCase,Mis_Type,[-6,6],'A-P relative motion (mm)','Foot-Strap Points')
            relativeMotionPlot(FSP_res_PD,theta_range,MCase,Mis_Type,[-10,10],'P-D relative motion (mm)','Foot-Strap Points')
            relativeMotionPlot(FP_res_AP,theta_range,MCase,Mis_Type,[-2,2],'A-P relative motion (mm)','Foot Points')
            relativeMotionPlot(FP_res_PD,theta_range,MCase,Mis_Type,[-10,10],'P-D relative motion (mm)','Foot Points')
        ...
        case 2 % Case-2 : Proximal-Distal Misalignments
            ...
            relativeMotionPlot(FSP_res_AP,theta_range,MCase,Mis_Type,[-10,10],'A-P relative motion (mm)','Foot-Strap Points')
            relativeMotionPlot(FSP_res_PD,theta_range,MCase,Mis_Type,[-6,6],'P-D relative motion (mm)','Foot-Strap Points')
            relativeMotionPlot(FP_res_AP,theta_range,MCase,Mis_Type,[-10,10],'A-P relative motion (mm)','Foot Points')
            relativeMotionPlot(FP_res_PD,theta_range,MCase,Mis_Type,[-2,2],'P-D relative motion (mm)','Foot Points')

        case 3 % Case-3 : Posterior Distal - Anterior Proximal Misalignment

            Mis_Type = 'Posterior Distal-Anterior Proximal Misalignments';
            SP_res_AP = zeros(9,5);
            SP_res_PD = zeros(9,5);
            TP_res_AP = zeros(9,5);
            TP_res_PD = zeros(9,5);
            ...
            relativeMotionPlot(TP_res_AP,theta_range,MCase,Mis_Type,[-10,10],'A-P relative motion (mm)','Touch Points')
            relativeMotionPlot(TP_res_PD,theta_range,MCase,Mis_Type,[-10,10],'P-D relative motion (mm)','Touch Points')
            relativeMotionPlot(FSP_res_AP,theta_range,MCase,Mis_Type,[-4,4],'A-P relative motion (mm)','Foot-Strap Points')
            relativeMotionPlot(FSP_res_PD,theta_range,MCase,Mis_Type,[-10,10],'P-D relative motion (mm)','Foot-Strap Points')
            relativeMotionPlot(FP_res_AP,theta_range,MCase,Mis_Type,[-10,10],'A-P relative motion (mm)','Foot Points')
            relativeMotionPlot(FP_res_PD,theta_range,MCase,Mis_Type,[-10,10],'P-D relative motion (mm)','Foot Points')
        ...
        otherwise % Case-4 : Posterior Proximal - Anterior Distal Misalignment
            ...
            Mis_Type = 'Posterior Proximal-Anterior Distal Misalignments';
            SP_res_AP = zeros(9,5);
            SP_res_PD = zeros(9,5);
            TP_res_AP = zeros(9,5);
            TP_res_PD = zeros(9,5);
            ...
            relativeMotionPlot(TP_res_PD,theta_range,MCase,Mis_Type,[-10,10],'P-D relative motion (mm)','Touch Points')
            relativeMotionPlot(FSP_res_AP,theta_range,MCase,Mis_Type,[-10,10],'A-P relative motion (mm)','Foot-Strap Points')
            relativeMotionPlot(FSP_res_PD,theta_range,MCase,Mis_Type,[-4,4],'P-D relative motion (mm)','Foot-Strap Points')
            relativeMotionPlot(FP_res_AP,theta_range,MCase,Mis_Type,[-10,10],'A-P relative motion (mm)','Foot Points')
            relativeMotionPlot(FP_res_PD,theta_range,MCase,Mis_Type,[-10,10],'P-D relative motion (mm)','Foot Points')
...
function illustrationPlot1(ACS,FE,FT,Knee,SP0_OCS,SP1_ACS,SP1_OCS,Knee_ACS,TP1_ACS,TP2_ACS,TP1_OCS,TP2_OCS,deltaX,deltaY,OCS)
...
% Function for Illustration Plots of Model-2
function illustrationPlot2(ACS,FE,FT,Knee,FSP0_OCS,FSP1_ACS,FSP1_OCS,FE_ACS,FT_ACS,FP1_ACS,FP2_ACS,FP1_OCS,FP2_OCS,deltaX,deltaY,OCS)
    if deltaX == -20 && deltaY == 0 % Posterior Misalignment Case
        figure(211);
        box on
        plot([ACS(1) Knee(1)],[ACS(2) Knee(2)],'color','k','LineWidth',1)
        ...
        plot([ACS(1),FE(1)],[ACS(2),FE(2)],'color','k','LineWidth',1)
        plot([FT(1),FE(1)],[FT(2),FE(2)],'color','k','LineWidth',1)
        plot([ACS(1),FE_ACS(1)],[ACS(2),FE_ACS(2)],'linestyle','--','color','k','LineWidth',1)
        plot([ACS(1),FT_ACS(1)],[ACS(2),FT_ACS(2)],'linestyle','--','color','k','LineWidth',1)
        plot([FE_ACS(1),FT_ACS(1)],[FE_ACS(2),FT_ACS(2)],'linestyle','--','color','k','LineWidth',1)
        plot([FSP0_OCS(1),FSP1_ACS(1)],[FSP0_OCS(2),FSP1_ACS(2)],'linestyle','-','color','b','LineWidth',1)
        plot([FSP0_OCS(1),FSP1_OCS(1)],[FSP0_OCS(2),FSP1_OCS(2)],'linestyle',':','color','r','LineWidth',1.5)
        plot([FP1_ACS(1),FP2_ACS(1)],[FP1_ACS(2),FP2_ACS(2)],'linestyle','-','color','c','LineWidth',1)
        plot([FP1_OCS(1),FP2_OCS(1)],[FP1_OCS(2),FP2_OCS(2)],'linestyle',':','color','m','LineWidth',1.5)
        plot(ACS(1),ACS(2),'y*','LineWidth',1.5)
        plot(OCS(1),OCS(2),'g*','LineWidth',1.5)
        xlim([-150,200]),ylim([-150,420])
        xticks([]),yticks([]);
        title('Posterior Offset')
    elseif deltaX == 20 && deltaY == 0 % Anterior Misalignment Case
        figure(212);
